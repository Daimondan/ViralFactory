...
**What:** Ratified DIVERGENCE-018 from direct operator instruction and bumped Charter v3.8 → v3.9. For composited media, the Assets stage now requires a Component Workbench before assembly. The system generates immutable candidates grouped by narration, visual media, soundtrack, SFX/source sound, typography, graphics, and config-declared format roles. The operator approves exact versions; deterministic completeness verifies every required role; a transaction freezes Writer/VO/module/config/component decisions into an immutable manifest; the assembler consumes that manifest only. Existing Gate 3 remains and separately approves the exact final artifact bound to the manifest hash.

**Runtime evidence:** The deployed service was active and `/health` returned 200, but database/runtime correlation showed 4 cards in `assembling`, 2 at `awaiting_soundtrack_approval`, 9 pending assets, 26 failed jobs, one dead job, and one running job. Card 59 / asset 18 remained at soundtrack approval for the full 25-minute observation with no final artifact. Recent jobs repeatedly progressed past one step and then failed on absent visuals or VO. Current `asset_media` has no generic category/candidate-version/approval/manifest identity; most recent generated rows can lack beat linkage. Current code also reduces a multi-platform draft to its first asset and exposes a Gate 3 route that can write approval without proving current final artifact + manifest + required evidence.

**Rationale:** A healthy process and passing component tests do not prove a piece can complete. The current chain couples generation, mutable inventory, a legacy soundtrack pause, planning, and render; it hides most taste-bearing choices until after stitching and cannot prove the assembler used only human-approved parts. Adding more route buttons would preserve the broken boundary. Candidate identity, human decision, completeness, manifest freeze, deterministic consumption, resumable orchestration, and final-artifact approval must be separate contracts.

**Soundtrack effect:** AMENDMENT-011's rights snapshots, local hashed acquisition, evidence-honest ranking, immutable mixes, cost approval, and exact-final-artifact Gate 3 remain binding. Its first-and-only soundtrack choice at Gate 3 is superseded. Soundtrack ingredient selection now occurs before assembly with an approved-VO-under-bed preview; Gate 3 still approves the exact final mix. These are distinct decisions.

**Build order:** Added M15 / VF-CW-001..012. VF-VS-515 is superseded. VF-VS-516 and VF-VS-702/703 remain blocked until a genuinely fresh deployed Reel proves candidate comparison, regeneration/invalidation, restart-safe human wait, immutable manifest, manifest-only assembly, alternative/new-manifest flow, Gate 3 invalidation/reapproval, and Gate 4 hold. Added the detailed plan, runtime review, architect note, and builder manifest. No production code changed in this architect pass.
...
**What:** Added PromotionStore with four deliberately distinct promotion actions per AMENDMENT-012 C5: Bookmark (keeps a reference without grounding ideation), Add to Source Bank (creates a status='new' source candidate linked to observation and collection provenance — the existing Source Bank Keep gate remains intact), Propose experiment (creates an async-gate proposal with evidence), and Propose pattern (creates a module proposal — approval required before a version bump). All promotion records are append-only with revert (undo preserves history). Bulk revert exists for queues that can exceed 50 items. Tenant scoping enforced on every action. Five API routes: bookmark, promote, list bookmarks, revert, bulk-revert. UI has four action buttons per card with confirmation that no module/source/idea/soundtrack was changed. PipelineStore._init_db initializes the PromotionStore schema.

**Rationale:** AMENDMENT-012 C5 requires that no observation silently changes a module, process, source status, idea input, or soundtrack. Each action's destination and undo/history are visible. Bookmark does not ground ideation. Source Bank promotion does not immediately feed ideation. Experiment and module proposals enter the async gate. The first slice's read-only boundary is preserved — promotion actions queue references and proposals, they do not execute changes.

**Verification:** 19 focused tests pass: bookmark creation and duplicate prevention, bookmark does not ground ideation (no source bank entry, no idea card), source bank promotion creates status='new' without feeding ideation, experiment and pattern proposals, no module change from proposal, revert preserves history, double-revert fails, tenant scoping, bulk revert, API routes (bookmark, promote, list, revert, bulk-revert), no silent idea creation, observation history unchanged by promotion. Full suite 2096 passed, 2 skipped.

### VF-INSP-004 — Deployed live-provider first-slice proof [TECH/STRUCTURE/OPS]

**What:** Ran one successful collection per enabled provider with real deployed credentials, then disabled provider network and proved /inspiration renders the persisted snapshot. Updated TikHub endpoints to the real API paths (which changed since the 2026-07-19 divergence filing): TikTok audio chart -> fetch_music_chart_list (scene param for Top 50/Viral 50), TikTok video search -> fetch_video_search_result (keyword param), IG reels -> search_reels (keyword param). Adapters now handle both fixture response shape and live response shape: TikTok audio uses nested data.music_list[].music_info with play_url as a dict; TikTok video uses nested data.search_item_list[].aweme_info with statistics/author/video/music; IG reels uses nested data.data.items[] with caption.text, user.username, video_versions. The standalone live smoke script expands chart configs, loads credentials from env, verifies no secrets reach the DB, and proves offline render. The runner sends provider-specific params (audioType, scene, keyword, first) and region only to non-TikHub providers (TikHub search rejects unknown params).

**Rationale:** AMENDMENT-012 C1 requires a deployed collection run to succeed with each enabled live provider without exposing secrets, and the page to render from the DB with provider network disabled. The first slice must remain useful when a provider is down: the failed provider shows an honest error state and prior data is preserved. The live smoke is separate from pytest so the automated suite never depends on live network.

**Verification:** Live proof: 3/4 providers collected successfully — Bundle.social Instagram audio (25 items), TikHub TikTok Top 50 + Viral 50 (50 items each), TikHub TikTok video search (20 items). TikHub Instagram reels returns HTTP 400 (provider plan limitation for Instagram) — the page shows an honest error state and prior data is preserved. Offline render proved with network blocked (zero provider calls, 323KB page rendered). 9 pytest tests pass: offline render with network blocked, auth failure non-destructive, plain-language errors, no credentials in DB or page, fixture-based collection completeness, live smoke script exists and is separate. Full suite 2077 passed, 2 skipped.

### VF-INSP-003 — Read-only top-level Inspiration UI [STRUCTURE/TECH/LOGIC]

**What:** Added Inspiration to the shared navigation between Home and Pipeline, and built the /inspiration route as a server-rendered Flask page reading from the database only (zero provider calls during render). The page has distinct audio (Trending audio) and video (Video inspiration) sections with truthful evidence labels per AMENDMENT-012 — chart evidence says "Trending audio," recommendation feeds say "Video inspiration," never "Top Trending Videos." Platform and region filters with visible current scope. All C7 states rendered: first-run, empty, stale, partial-provider-failure, all-provider-failure, and unavailable-media. Long descriptions are expandable (Show more toggle), not silently truncated. Playable media appears only when a safe preview URL exists; an unavailable state otherwise. Missing metrics render as unavailable, not zero. No green success state for a failed or stale collection. First-observation disclaimer explains that a single observation is not proof of momentum. No ideation, publish, download, source-bank, or experiment actions — the first slice is read-only. Manual refresh queues a non-blocking job; it never makes the web request wait on a provider.

**Rationale:** AMENDMENT-012 C1 requires the page to read persisted observations from the database and never call provider APIs during render. C7 requires truthful evidence headings, every state with a clear next action, no false-green, expandable content, and no secrets or raw provider jargon rendered to the operator. The first slice cannot feed ideation, modules, or production.

**Verification:** 23 focused tests pass: nav placement (Inspiration before Pipeline) and active state, instrumented zero-provider-call render (requests.get and _http_get patched to fail), truthful labels (chart=Trending audio, recommendation=Video inspiration, NOT Top Trending Videos), card content (platform/provider/creator/title/collection time), preview link when available + No preview message when unavailable, missing metrics show unavailable, long captions have Show more toggle, platform filters visible and functional, first-run state when no data, stale state when data old, no secrets in page, no ideation/publish/download/source-bank/experiment actions, refresh queues not blocks, no false-green for stale, first-observation disclaimer, rank display. Full suite 2068 passed, 2 skipped.
...
**What:** Defined the normalized evidence contracts for the Inspiration workbench (AMENDMENT-012 C2/C3): collection-run (provider, endpoint key, platform, region, sanitized params, start/end, status, result count, response hash, adapter/config version, redacted error), trend-item (provider-native identity, content type, canonical URL, creator/title/description, safe preview/thumbnail, availability), and append-only observation (run link, item link, collected time, rank, exact metric names/values, evidence label, immutable safe-payload hash). Added URL and payload redaction that strips signed query params and secret-bearing fields before persistence, with a stable SHA-256 safe-payload hash independent of stripped secrets. Metric normalization preserves missing values as unavailable (never zero). Added config/inspiration.yaml with tenant-scoped provider enablement, endpoint meaning (chart vs recommendation), regions, content types, env-var credential references, TTLs, request budgets, redaction rules, and truthful section labels (chart → Trending audio; recommendation → Video inspiration). Added 9 redacted fixtures covering chart, recommendation, seed, empty, partial, malformed, rate-limited, and auth-failed states.

**Rationale:** Inspiration is a read-only evidence workbench, not a fifth profile and not a soundtrack shortcut. Provider facts must travel with provider, time, region, endpoint meaning, and exact metric — never relabeled by Python. Recommendation feeds may not claim trend velocity. Secrets and signed URLs must never reach the DB. The first slice contains no LLM judgment and no action that feeds ideation, modules, or production.

**Verification:** 52 focused tests pass: contract validation (missing/empty/invalid fields, statuses, content types, hashes, evidence labels), redaction (case-insensitive URL params, nested payload recursion, malformed URLs), safe-hash stability across key order, fixture validation through contracts for all 9 states, config truthfulness (no business values, no secrets, env-var refs only, truthful section labels), and a guard that the contracts module imports no network libraries. No live network or credentials in the automated suite.
...
**Rationale:** A gate class without production routes, listening evidence, or render enforcement cannot prevent unreviewed music from publishing. Binding every human decision to the server-resolved current plan closes stale-ID and caller-authored-token bypasses. Keeping changed plans immutable and requiring fresh approval preserves auditability, while a normal autonomous pause avoids misclassifying required operator judgment as a production failure. Source-sound resolution and path normalization remain mechanical; no genre choice, random effect, or synthetic sound judgment was added to Python.

**Verification:** 26 focused gate tests cover append-only decisions, server-minted tokens, exact-current-plan ownership, stale-approval rejection, positive and cross-owner replacement, explicit VO-only, required preview acknowledgment, source-sound resolution, actual FFmpeg bed-under-VO output, render blocking, autonomous pause/resume, real Flask routes, and page controls. Real-server curl proved HTTP 409 before listening, persisted acknowledgment, approval, and approved readback. Full linked-worktree suite: `1,869 passed, 7 skipped`.
...
**Rationale:** The prior shared production path bypassed the ratified M13 components: it told the planner there was no VO take, never invoked `CueCompiler`, accepted a legacy shape, and saved no proof linking the plan to approved words or measured audio. A green isolated compiler test therefore proved nothing about live production. The repaired boundary makes VO the master clock and limits code to deterministic validation and translation; source selection remains an LLM judgment and all presentation values remain config-driven.

**Verification:** 20 VF-AU-206 tests cover exact measured timing and VO artifact persistence/renderer consumption, legacy-only VO fallback, exact caption-word preservation, real inventory IDs and stock aliases, config-driven styles, scoped source hashing, per-beat compliance timing, persisted provenance/contracts, missing-VO fail-closed behavior, duration mismatch rejection, missing/zero/invalid/video source bounds, required beat coverage, cue resolution, duplicate IDs, and invalid source rejection. 117 related route/chain/assembly/audio tests pass. Full-suite equivalent: 1,837 passed and 7 skipped; the 10 deliberately cwd-bound real-data tests were run from the primary repo against the same worktree source because linked worktrees do not duplicate ignored `data/` artifacts.
...
2. `check_talking_head_motion_coverage(beats, vo_segments, motion_durations)` — if a beat's `visual_intent` describes talking-head (person speaking to camera) and available motion is shorter than the VO span beyond `MOTION_SHORTFALL_THRESHOLD` (50%), and there is no explicit cutaway event (a second visual_event with a non-generated source_policy), it is blocked. This catches the Draft 8 Artifact A defect: 14s BUILD beat with 5s motion + silent still fallback.
Both wired into `run_feasibility_checks` via new optional params `beats`, `vo_segments`, `motion_durations`. When `beats=None` (backward compat), the new checks are skipped.
**Why:** The Draft 8 Artifact A defect — a 14s talking-head beat with only 5s of motion, silently padded with a still — reached the operator because no feasibility check validated event coverage or motion-vs-VO span. The amendment requires these to be caught before render.
**Rationale:** Coverage and motion-shortfall are mechanical checks (arithmetic on time ranges), not judgment — they belong in deterministic code, not LLM. The talking-head detection is a mechanical pre-check using semantic markers from `visual_intent`; the LLM compliance contract remains authoritative for compliance decisions. The shortfall threshold (50%) and coverage tolerance (0.25s) are module-level constants, not business values.
**Verification:** 21 new tests cover: full coverage pass, gap/overlap/incomplete/out-of-bounds detection, no-events skip, measured-VO override, tolerance, talking-head short motion blocked (Draft 8 pattern), enough motion pass, cutaway pass, non-talking-head not checked, small shortfall tolerance, measured-VO for talking-head, runner integration, backward compat. 55 tests across feasibility + compliance suites green. Full suite 1,712 tests green in 165s.

### VF-VS-402 — Visual Director process registered [STRUCTURE]
**What:** Added `prompts/assembly/visual_director_v1.md` (v1.0) — an Assembler-side LLM process that translates the Writer's `visual_intent` + measured VO timings into concrete `visual_events[]`. Added `VISUAL_DIRECTOR_SCHEMA` to `pipeline.py` (beats array, each with beat_id + visual_events; event enums match the production contract's `VISUAL_EVENT_*` enums). Registered `visual_director_v1` in `config/processes.yaml` with `playbook_type: production`, `backend: drafter`, `schema: VISUAL_DIRECTOR_SCHEMA`, inputs: contract_beats, vo_timeline, visual_style. Added `VISUAL_DIRECTOR_SCHEMA` to the schema registry mapping.
**Why:** Draft 8's 14s BUILD beat had 5 visual events that the one-beat-one-visual model could not represent. The Visual Director moves visual planning judgment to a prompt + schema + validator (AMENDMENT-010 Condition 5), not keyword heuristics or arbitrary cadence rules. It is Assembler-side: it produces production planning, not audience copy.
**Rationale:** Visual planning is judgment work (when to split a beat, what narrative function each event serves) — it belongs in a registered, versioned, provenance-logged LLM process, not in hardcoded Python. The prompt enforces: no audience copy, no tenant strings, no provider names, honor the VO clock, honor capture policy. `playbook_type: production` keeps it out of the onboarding UI.
**Verification:** 18 new tests cover schema shape, enum matching with the contract, valid/invalid output validation, prompt file existence/version/tenant-string-free/provider-name-free/Assembler-side-boundary, registry entry presence/correct fields/playbook_type/not-retired/schema resolution/inputs. 39 tests across VF-VS-402 + process registry suites green. Full suite 1,695 tests green in 167s.

### VF-VS-401 — visual_events[] added to production contract [STRUCTURE]
**What:** `PRODUCTION_CONTRACT_V2` beat schema gains `visual_events[]` per AMENDMENT-010 Condition 5. New `VISUAL_EVENT_SCHEMA` (event_id, time_range, narrative_function, source_policy, required_text, capture_policy_ref) with two new enums: `VISUAL_EVENT_NARRATIVE_FUNCTIONS` (hook_contrast, context, proof, explanation, reframe, action, landing, relationship, conflict) and `VISUAL_EVENT_SOURCE_POLICIES` (operator_capture, licensed_stock, approved_reference, generated_still, generated_motion, renderer_graphic). New `validate_visual_events()` recurses into array items (the top-level validator doesn't), checking required fields, enum values, time_range start<=end, and duplicate event_id within a beat. Wired into `assemble_contract`. New `resolve_visual_events(beat)` degrades old beats: no visual_events → one synthesized event from visual_intent (context function, capture-policy-mapped source_policy).
**Why:** Draft 8's 14s BUILD beat had 5 visual events; the one-beat-one-visual model cannot represent this. The amendment adds the array and a graceful degradation so existing contracts keep working.
**Rationale:** Visual events are Assembler-side production planning (concrete visual jobs), not audience copy. The schema is additive — old beats without the field still validate. The resolver centralizes the "no events → synthesize one" rule so downstream services (VF-VS-402 Visual Director, VF-VS-403 feasibility) call one function.
**Verification:** 18 new tests cover schema shape, enum membership, multi-event validation (Draft 8's 5 events), invalid enum/missing-field/time-range/duplicate rejection, full-contract assembly with events, full-contract rejection on invalid event, and graceful degradation (old beat → one context event, capture-policy mapping for all 7 policies). 79 tests across contract/compat/validator suites green. Full suite 1,677 tests green in 168s.
...
**Verification:** 5 new delegation tests prove equivalence with the shared service, 3–5 bounds, no dangling tails, exact reconstruction, and empty handling. 83 tests across episode/caption suites green. Full suite 1,659 tests green in 168s.
...
**Verification:** 7 new multi-phrase tests (long caption splits, within-span, contiguous, short-stays-single, multi-beat, metadata, blank). 49 tests across caption/cue/integration suites green. Full suite 1,659 tests green in 168s (run with VF-VS-303).
