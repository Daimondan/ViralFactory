# BUILD_PLAN.md — StackPenni System

*Instructions to the builder agent (Hermes, open-source models). Read `docs/CHARTER-v3.md` and all of `playbooks/` before writing any code. This file is the single source of truth for what to build and in what order. v1.0 — 2026-07-01*

## How to work (non-negotiable)

1. **One task at a time, top-down.** Complete a task, check its box, commit with the task ID in the message, append one line to `PROGRESS.md` (date · task ID · what changed · any open question). Never batch tasks in one commit.
...
4. **When uncertain, ask — in the repo.** Open an issue. Do not guess at design intent; the charter and playbooks are the intent.
...
- **No judgment in code.** Anything requiring understanding (tagging, titling, summarizing, quality calls, voice analysis) is an LLM step: prompt template in `prompts/` + JSON output schema + validator. Never regex/keyword heuristics for judgment — this rule exists because keyword matching once falsely tagged 65 of 80 sources.
- **Mechanics use boring libraries.** Content extraction/boilerplate stripping = trafilatura (or equivalent). Do not spend LLM calls on mechanical work.
...
- **No new patch scripts.** If output is wrong, the fix is in the prompt, config, or validator — committed and versioned — never a one-off SQL/script fix.
- **Small PRs, tested.** Each task lands with at least one test. Keep the existing 68 tests green.

## Stack (fixed unless the charter changes)

- Python + Flask console (extend the existing app), SQLite (existing), systemd on the VPS
...
- trafilatura for extraction · Postiz (self-hosted) for publish/metrics · OB1 (Obsidian) as the knowledge layer for user #1; modules also mirrored as markdown in `modules/{business}/`
- Scheduling: cron (keep it boring)

## Milestones

### M0 — Foundations (est. week 1)
- [ ] T0.1 Repo layout: `config/ prompts/ playbooks/ modules/ src/ tests/ docs/` — AC: matches charter structure; README maps folders to charter concepts
- [ ] T0.2 Config loader: `business.yaml`, `models.yaml`, `sources.yaml` with schema validation — AC: bad config fails loudly with a clear message; no defaults hidden in code
...
- [ ] T1.3 Voice Profile playbook end-to-end (`playbooks/voice-profile-builder.md`) — AC: from real materials to draft profile matching the output schema; every finding carries verbatim evidence (validator enforces)
...
- [ ] T3.3 Reaction UI: per-line reactions in plain words → Feedback Log; revise → ship/kill — AC: every reaction stored with the draft version it applied to
- [ ] T3.4 Manual publish handoff (copy/queue) — AC: shipped pieces exportable per platform format from Format Guide
...
- [ ] T5.1 Weekly proposal job: results + Feedback Log → LLM analysis → proposed module updates, each with evidence + target module + exact diff — AC: proposals are specific and actionable, not vibes ("question-openers beat statements 4/5 — add to Viral Patterns §Hooks")
- [ ] T5.2 Gate UI: approve/reject per proposal; approve = version bump with provenance — AC: one-sitting review; rejected proposals logged with reason
- [ ] T5.3 Voice Profile update path from Feedback Log per playbook update procedure — AC: an approved reaction pattern lands in the profile as a new versioned entry
...
- [ ] T6.3 Proposals + **Experiments Queue**: untried formats become proposed experiments at the gate; approved experiments appear as seed suggestions — AC: an approved experiment flows into Pick + seed
...
### M7 — Generalization proof (est. week 11+)
- [ ] T7.1 Onboard a second business (test brand) entirely through the console — AC: **zero code changes**; all differences live in config + modules
