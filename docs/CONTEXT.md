...
> **This is an operational mirror of `docs/CHARTER-v3.2.md`.** It captures
...
**Updated:** 2026-07-02
**Conforms to:** `docs/CHARTER-v3.2.md` (v3.2 — incorporates DIVERGENCE-001, DIVERGENCE-002, and AMENDMENT-003 staged content pipeline)
...
### Idea card
The first artifact in the staged pipeline. Each card carries: the idea, hook/title options, suggested format, origin tag (`ai-originated` | `human-seeded` | `human-seeded-ai-developed`), and evidence links. Cards are generated by crossing Source Bank items with the living modules (AI-originated) or from the person's raw seed. Gate 1 (rigorous: approve/kill/park) decides which cards proceed to Draft.

### Origin
The provenance tag that travels with a piece from idea card to Results: `ai-originated`, `human-seeded`, or `human-seeded-ai-developed`. The nightly performance note records it so the inward loop can answer: do the operator's seeds outperform AI-originated ideas? Instrumented from the first piece.
