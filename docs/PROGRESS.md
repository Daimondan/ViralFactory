...
**Current Phase:** T10.7+T10.10 complete, T11.2+T11.4 complete. M10 compliance loop fully done. 1,431 tests passing. Next: T11.5 (episode-format module + bootstrap), T11.6 (EpisodePlan schema + Writer beats), T11.7 (storyboard gate).
...
## What's Done
- [x] 2026-07-17: T11.4 — fal provider in MediaAdapter. `generate_image(reference_images=[...])` uploads local files to fal storage and passes URLs as reference conditioning. `submit_video(mode="image_to_video", source_image=...)` uploads source still and submits image-to-video job. `check_fal_job()` polls via fal_client.status/result. All endpoints read from `config/models.yaml` — zero hardcoded. Cost computed from config `cost_per_image_usd` / `cost_per_second_usd`. Provenance logged with `provider="fal"`. 17 mock-based tests (no real API calls). 1,431 total passing.
