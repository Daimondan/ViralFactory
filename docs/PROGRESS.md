...
**Current Phase:** Charter v3.12 is current. M16 implementation through VF-VIS-1617 is complete; VF-PROOF-1618 remains open on mobile emulation/overflow, truncated VO, Buffer contract failure, and a timed-out full suite. AMENDMENT-020 adds M17 as the next product direction after those truthfulness blockers become the legacy baseline: one persistent Story Room per piece, additive storage/locks first, then prompt-backed collaboration and Desk/Story UI, then three controlled pieces and an operator cutover gate. The legacy pipeline remains default/fallback; no destructive migration or nav cutover is approved. M14's Inspiration checkpoint, review-w8, review-episode-format, and M7 remain pending unless explicitly absorbed by later proof.
