...
2026-07-23 · VF-CW-003 · Config + prompt-driven component requirements. Added config/component_categories.yaml (6 generic categories: narration, visual_media, soundtrack, sound_effects, typography, graphics — no business values), prompts/assembly/component_requirements_v1.md (LLM planner), src/services/component_requirements.py (ComponentCategoryRegistry, COMPONENT_REQUIREMENTS_SCHEMA, ComponentRequirementsValidator, ComponentRequirementsStore). Registered in processes.yaml as component_requirements_v1. Python validates IDs/cardinality/references only — no keyword creative judgment. Two format fixtures (reel vs thread) yield different valid role sets with zero Python edits. Required-real capture flag prevents generated substitution. Append-only versions with content-hash and provenance. 20 new tests. 2159 passed, 2 skipped · Q: none
