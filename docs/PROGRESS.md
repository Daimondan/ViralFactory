...
2026-07-23 · VF-CW-004 · Immutable candidates + append-only decisions. Added CandidateStore (src/services/candidate_store.py) with: stable candidate lineage (hash-based ID from tenant+session+category+role+scope), immutable append-only versions, exact artifact/preview hashes, source/generation provenance, rights/cost links, status lifecycle (generating→available→approved|rejected→superseded|stale|failed), append-only decisions (select/approve/reject/regenerate) bound to candidate_version+artifact_hash+requirement_version_hash. Superseded/failed/stale candidates cannot be approved. No single 'approved' Boolean — status + decision history are authoritative. Cross-session isolation. 15 new tests. 2174 passed, 2 skipped · Q: none
